...
def get_training_rays_in_maskcache_sampling_sr(
    ...
    for c2w, img, (H, W), K in zip(train_poses, rgb_tr_ori, HW, Ks):
        ...
        rgb_tr_curr = torch.zeros([n,3], device=DEVICE)
        rays_o_tr_curr = torch.zeros_like(rgb_tr_curr)
        rays_d_tr_curr = torch.zeros_like(rgb_tr_curr)
        viewdirs_tr_curr = torch.zeros_like(rgb_tr_curr)

        rgb_tr_curr.copy_(img[mask])
        rays_o_tr_curr.copy_(rays_o[mask].to(DEVICE))
        rays_d_tr_curr.copy_(rays_d[mask].to(DEVICE))
        viewdirs_tr_curr.copy_(viewdirs[mask].to(DEVICE))

        rgb_tr_lst.append(rgb_tr_curr)
        rays_o_tr_lst.append(rays_o_tr_curr)
        rays_d_tr_lst.append(rays_d_tr_curr)
        viewdirs_tr_lst.append(viewdirs_tr_curr)
    ...
    eps_time = time.time() - eps_time
    print('get_training_rays_in_maskcache_sampling: finish (eps time:', eps_time, 'sec)')
    return rgb_tr_lst, rays_o_tr_lst, rays_d_tr_lst, viewdirs_tr_lst, imsz
