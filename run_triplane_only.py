...
from lib import utils, triplane_only_dvgo, dmpigo, ray_utils
...
def compute_bbox_by_coarse_geo(model_class, model_path, thres):
    ...
    density = model.density.reshape(dense_xyz.shape[:-1])
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if reload_ckpt_path is None:
        ...
        if cfg.data.ndc:
            ...
            if len(cfg_train.pg_scale) and reload_ckpt_path is None:
                num_voxels = int(num_voxels / (2**len(cfg_train.pg_scale)))
            model = triplane_only_dvgo.DirectVoxGO(
                xyz_min=xyz_min, xyz_max=xyz_max,
                num_voxels=num_voxels,
                mask_cache_path=coarse_ckpt_path,
                **model_kwargs)
        ...
        if cfg.data.ndc:
            ...
            model_class = triplane_only_dvgo.DirectVoxGO
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if model.mask_cache is not None and (global_step + 500) % 1000 == 0:
            if model.density is None or not hasattr(model, 'density'):
                tqdm.write('computing model density')
                model.get_model_density()
            ...
            tqdm.write('masking empty space done')

        # progress scaling checkpoint
        if global_step in cfg_train.pg_scale:
            n_rest_scales = len(cfg_train.pg_scale)-cfg_train.pg_scale.index(global_step)-1
            cur_voxels = int(cfg_model.num_voxels / (2**n_rest_scales))
            if isinstance(model, triplane_only_dvgo.DirectVoxGO):
    ...
    if global_step != -1:
        ...
        torch.save({
            ...
            'k0_zx_state_dict': model.k0_zx
        }, k0_ckpt_path)
        print(f'scene_rep_reconstruction ({stage}): saved checkpoints at', k0_ckpt_path)
    ...
    if cfg.data.ndc:
        ...
        if cfg.fine_model_and_render.use_coarse_geo:
            xyz_min_fine, xyz_max_fine = compute_bbox_by_coarse_geo(
                    model_class=triplane_only_dvgo.DirectVoxGO, model_path=coarse_ckpt_path,
...
if __name__=='__main__':
    ...
    if args.export_coarse_only:
        ...
        with torch.no_grad():
            ...
            model = utils.load_model(triplane_only_dvgo.DirectVoxGO, ckpt_path).to(device)
