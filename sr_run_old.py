...
from lib import utils, sr_dvgo, dmpigo, ray_utils
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses[:10])):
        ...
        for chunck in range(n_chunks):
            ...
            render_result_dict = model(None, ro, rd, vd, global_step=chunck, **render_kwargs)
    ...
    if cfg.data.ndc:
        ...
        if cfg.fine_model_and_render.use_coarse_geo:
            xyz_min_fine, xyz_max_fine = compute_bbox_by_coarse_geo(
                    model_class=sr_dvgo.DirectVoxGO, model_path=coarse_ckpt_path,
