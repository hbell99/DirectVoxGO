...
from lib import utils, triplane_dvgo, dmpigo, ray_utils
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if reload_ckpt_path is None:
        ...
        if cfg.data.ndc:
            ...
            if len(cfg_train.pg_scale) and reload_ckpt_path is None:
                num_voxels = int(num_voxels / (2**len(cfg_train.pg_scale)))
            model = triplane_dvgo.DirectVoxGO(
                xyz_min=xyz_min, xyz_max=xyz_max,
                num_voxels=num_voxels,
                mask_cache_path=coarse_ckpt_path,
                **model_kwargs)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            ...
            if isinstance(model, triplane_dvgo.DirectVoxGO):
    ...
    if cfg.data.ndc:
        ...
        if cfg.fine_model_and_render.use_coarse_geo:
            xyz_min_fine, xyz_max_fine = compute_bbox_by_coarse_geo(
                    model_class=triplane_dvgo.DirectVoxGO, model_path=coarse_ckpt_path,
...
if __name__=='__main__':
    ...
    if args.export_coarse_only:
        ...
        with torch.no_grad():
            ...
            model = utils.load_model(triplane_dvgo.DirectVoxGO, ckpt_path).to(device)
